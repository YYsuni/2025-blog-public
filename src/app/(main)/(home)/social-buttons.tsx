...
export default function SocialButtons() {
  ...
  const styles = resolveHomeCardFrame(cardStyles, 'socialButtons', mode)
  ...
  const buttonRefs = useRef<Record<string, HTMLButtonElement | HTMLAnchorElement | null>>({})

  useEffect(() => {
    const baseDelay = styles.order * ANIMATION_DELAY * 1000
  ...
  }, [delay, sortedButtons, styles.order])
  ...
  const iconMap: Record<SocialButtonType, React.ComponentType<any>> = {
    github: GithubSVG,
    juejin: JuejinSVG,
    email: EmailSVG,
    wechat: WechatSVG,
    ...
    if (button.type === 'github') {
      return (
        <motion.a
          ...
          {...commonProps}
          className='card btn flex items-center gap-2 rounded-xl p-1.5'
        >
                ...
                createPortal(
                  isOpen ? (
                    <>
                      <button
                        onClick={() =>
                          setOpenDropdowns(prev => ({ ...prev, [button.id]: false }))
                        }
    ...
    if (button.type === 'link') {
      return (
        <motion.a
          ...
          {...commonProps}
          className='card btn rounded-xl px-3 py-1.5 text-sm'
        >
  ...
  return (
    ...
    >
      <div
        className='absolute z-[6] flex flex-wrap items-center gap-2'
        style={{ left: x, top: y, width: styles.width, minHeight: styles.height }}
      >
